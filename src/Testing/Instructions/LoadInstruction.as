package Testing.Instructions {
	...
	public class LoadInstruction extends RegInstruction {
		...
		override public function execute(memory:Dictionary, registers:Dictionary):int {
			if (isNaN( memory[registers[args[2].value]]))
				throw new Error("!!!");
			registers[args[0].value] = memory[registers[args[2].value]];
			return C.INT_NULL;
		}
		
		override public function toMemValue():InstructionValue {
			return new InstructionValue(OpcodeValue.OP_LD, C.INT_NULL, args[2].value, args[0].value, abstract.toString(), abstract.toFormat());
		}
